...
export default function EditShortcut({ shortcut, icon, close, setShortcuts }) {
    ...
    async function handleSubmit(e){
        ...
        try {
            ...
            const send = await updateRecordField("shortcuts", { id: formData.shortcut.toUpperCase(), shortcut: formData.shortcut.toUpperCase(), icon: newIcon });
            if(send === true){
                setTimeout(() => {
                    setShortcuts(oldShorts => {
                        const newShortcuts = oldShorts.filter(short => short.shortcut.toUpperCase() !== shortcut.toUpperCase());
                        newShortcuts.push({ shortcut: formData.shortcut.toUpperCase(), id: formData.shortcut.toUpperCase(), icon: newIcon });
        ...
        <div className='confirm-action-wrapper'>
            <form className='action-form' onSubmit={handleSubmit}>
                ...
                <div className="action-main">
                    ...
                    <input type="file" readOnly={ loading } className='action-input' name="icon" onChange={handleChange} accept='image/*' style={{ marginTop: "-5px" }} />
