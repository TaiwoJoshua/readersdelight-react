        ...
        try {
            ...
            if(send === true){
                setTimeout(() => {
                    setKeywords(oldKeys => {
                        const newKeywords = {};
                        const newKeys = Object.keys(oldKeys).filter(key => key.toUpperCase() !== keyword.key.toUpperCase());
        ...
        <div className='confirm-action-wrapper'>
            <form className='action-form' onSubmit={handleSubmit}>
                ...
                <div className="action-main">
                    ...
                    <input type="text" name="key" readOnly className='action-input' placeholder="Keyword" maxLength={10} minLength={3} value={formData.key} onChange={handleChange} autoComplete="off" style={{ textTransform: "uppercase", marginBottom: "-5px", fontWeight: "bolder" }} required pattern="[A-Za-z]+" />
                    <input type="text" name="fullword" readOnly={loading} className='action-input' placeholder="Fullword" maxLength={20} minLength={3} value={formData.fullword} onChange={handleChange} autoComplete="off" style={{ textTransform: "uppercase" }} required pattern="[A-Za-z ]+" />
