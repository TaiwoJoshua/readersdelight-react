...
export default function KeywordsList() {
    ...
    const elements = Object.keys(keywords).sort().map(key => {
        const fullword = keywords[key];
        return <span key={nanoid()} className='shortcut-card'><span><strong>{ key }</strong></span><span className='shortcut-options one'  onClick={() => { setEditer({ key, fullword }); setEditClose(false); }}><FaFilePen /></span><span className='shortcut-options two' onClick={() => { setDeleter(key); setClose(false); }}><FaTrash /></span></span>
    });

    return (
        <div className='shortcuts-wrap special keywords-wrap'>
            {!close && <DeleteKeyword keyword={ deleter } close={() => {setClose(true)}} setKeywords={ setKeywords } />}
            {!editClose && <EditKeyword keyword={ editer } close={() => {setEditClose(true)}} setKeywords={ setKeywords } />}
            {!newClose && <NewKeyword close={() => {setNewClose(true)}} setKeywords={ setKeywords } />}
