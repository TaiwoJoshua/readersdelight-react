...
export function randomBooks() {
  function generateRandomData() {
    ...
    function generateRandomSize() {
      const courses = ["MB", "KB"];
      return `${Math.floor(Math.random() * 150)}.00${
        courses[Math.floor(Math.random() * courses.length)]
      }`;
    }
...
export function randomRequests() {
  function generateRandomData() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const courses = generateRandomCourses();
      const status =
        statusOptions[Math.floor(Math.random() * statusOptions.length)]; // Randomly select status
      const timestamp = generateRandomTimestamp(); // Generate random timestamp
...
export function sortBooks(
  ...
  const sortedArrayByTitle = sortArrayByWordsInCommon(
    data,
    search,
    keywords,
    false,
  ...
  const sortedArrayByCourses = sortArrayByWordsInCommon(
    data,
    search,
    keywords,
    false,
  ...
  const order = {
    title: sortedArrayByTitle,
    author: sortedArrayByAuthor,
    courses: sortedArrayByCourses,
  };
  ...
  const length =
    ...
    sortedArrayByCourses.length +
    sortedArrayByTitle.length;
